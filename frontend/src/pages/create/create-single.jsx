...
import { addSingleItem } from "../../utils/contracts";
...
function CreateSingle(props) {
    ...
    function fetchCollections() {
        ...
        axios.get(`${process.env.REACT_APP_API}/collection?ownerAddress=${account}&type=single&sortDir=asc`)
            .then((res) => {
                if (res.data.status) {
    ...
    return (
        <div>
            ...
            <Element.Container>
                ...
                {account && active &&
                    <>
                        <Element.Title><h1 className={`text_color_gradient_${theme}`}>Create single NFT</h1></Element.Title>
                        <Element.UploadField>
                        ...
                        <Element.UploadField style={{ display: showCoverUpload ? "" : "none" }}>
                            <Element.Label className={theme}>Upload cover</Element.Label>
